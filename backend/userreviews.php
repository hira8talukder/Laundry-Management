...
<head>
  ...
  <title>User Reviews - Smart Laundry</title>
...
<body>
  ...
  <script>
    ...
    function filterReviews() {
      const isAll = document.getElementById("allCheckbox").checked;
      const selectedMonth = document.getElementById("monthPicker").value;
      const reviews = document.querySelectorAll(".review-box");

      reviews.forEach((review) => {
        const reviewDate = review.querySelector(".date").dataset.raw; // e.g. 2025-06-26
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      document.getElementById("allCheckbox").addEventListener("change", () => {
        document.getElementById("monthPicker").disabled = document.getElementById("allCheckbox").checked;
      });
...
fetch("../backend/get_user_reviews.php")
  .then(res => res.json())
  .then(reviews => {
    reviews.forEach(r => {
      const reviewHTML = `
        <div class="review-box">
          ...
          <div class="date" data-raw="${r.review_date}">${new Date(r.review_date).toLocaleDateString()}</div>
        </div>
      `;
